...
class BoreasPlotter:
    ...
    """
    def __init__(self, timestamps, curr_ts_idx, transforms, lidar_scans=None, camera_data=None, labels=None, mode='both'):
        # Data
    ...
    def update_plots(self, frame_idx):
        ...
        if self.mode == 'persp' or self.mode == 'both':
            curr_camera_img = self.camera_data[frame_idx]
    ...
    def update_plot_persp(self, ax, camera_img, lidar_scan, downsample_factor=0.3):
        if lidar_scan is None:  # If there is no lidar points, just show image
            ax.imshow(camera_img)
        else:
            points = lidar_scan.points[:, 0:3]
