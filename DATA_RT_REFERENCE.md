...
The beta parameter from the paper above is stored in `calib.radar_doppler_beta`.
...
For convenience, we also provide a way to computed a Cartesian representation of each radar scan. A trimmed polar (left) and Cartesian representation (right) of a radar scan are visualized below:
