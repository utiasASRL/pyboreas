...
def get_sequence_poses_gt(path, seq):
    ...
    for filename in seq:
        ...
        filepath = os.path.join(path, dir, 'applanix/aeva_poses.csv')  # use 'aeva_poses.csv' for groundtruth
...
def eval_odom(pred, gt):
    ...
    # compute errors
    t_err, r_err, _ = compute_kitti_metrics(T_gt, T_pred, seq_lens_gt, seq_lens_pred, seq, pred, 3, None)
